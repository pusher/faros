	...
	Context("With --namespaced-only=true", func() {
		...
		Context("When a GitTrack resource is created", func() {
			Context("with a cluster scoped resource", func() {
				...
				It("does not create any ClusterGitTrackObjects", func() {
					cgtos := &farosv1alpha1.ClusterGitTrackObjectList{}
					Consistently(func() *farosv1alpha1.ClusterGitTrackObjectList {
						err := c.List(context.TODO(), cgtos)
						if err != nil {
							return nil
						}
						return cgtos
					}, consistentlyTimeout).Should(testutils.WithItems(BeEmpty()))
				})
			})
		})
	})
