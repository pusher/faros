...
var _ = Describe("GitTrackObject Suite", func() {
	...
	Context("with --namespaced-only=true", func() {
		BeforeEach(func() {
			farosflags.NamespacedOnly = true
			Expect(add(mgr, recFn)).NotTo(HaveOccurred())
		...
		AfterEach(func() {
			// Make sure to reset any flags we change
			farosflags.NamespacedOnly = false
		})
		...
		Context("ReconcileGitTrackObject", func() {
			Context("with a ClusterGitTrackObject", func() {
				...
				Context("with valid data", func() {
					var copyGTO *farosv1alpha1.ClusterGitTrackObject

					BeforeEach(func() {
						// Create and fetch the instance to make sure caches are synced
						m.Create(gto).Should(Succeed())
						// Copy the GTO now we have initialised it, but before we refetch it
						copyGTO = gto.DeepCopy()
						// TypeMeta normally filled by Get()
						copyGTO.TypeMeta = metav1.TypeMeta{
							Kind:       "ClusterGitTrackObject",
							APIVersion: "faros.pusher.com/v1alpha1",
						}
					})

					It("should not create the child resource", func() {
						m.Get(child, consistentlyTimeout).ShouldNot(Succeed())
					})

					It("should not update the cgto", func() {
						m.Consistently(gto, consistentlyTimeout).Should(Equal(copyGTO))
					})
				})
			})
		})
