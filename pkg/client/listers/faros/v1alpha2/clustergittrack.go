...
type ClusterGitTrackLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.ClusterGitTrack, err error)
	// Get retrieves the ClusterGitTrack from the index for a given name.
	Get(name string) (*v1alpha2.ClusterGitTrack, error)
...
// List lists all ClusterGitTracks in the indexer.
func (s *clusterGitTrackLister) List(selector labels.Selector) (ret []*v1alpha2.ClusterGitTrack, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.ClusterGitTrack))
	})
	return ret, err
}

// Get retrieves the ClusterGitTrack from the index for a given name.
func (s *clusterGitTrackLister) Get(name string) (*v1alpha2.ClusterGitTrack, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("clustergittrack"), name)
	}
	return obj.(*v1alpha2.ClusterGitTrack), nil
}
